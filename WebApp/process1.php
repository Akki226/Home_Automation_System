...
foreach ($data[0]['rooms'] as $key => $room) {
    if ($room != null) {
        ...
        $returner['roomsdata'] .=  "<tr><td class='text-bold' style='color:{$color}'>Room {$key}</td><td style='visibility:{$disp};'><a href='logoData.php?room={$key}'>Show</a></td></tr>\n";
    }
}
...
<div class='card-body'>
    ...
    foreach ($rooms_data['heaters'] as $key => $row) {
        if ($row != null) {
            ...
            $returner['data'] .=  "<tr class='text-bold'><td>Heater {$key} </td><td>Wattage={$row['Wattage']}Watts</td><td>Runtime={$row['runtime']}Seconds</td><td>Energy Cost={$row['energyCost']}p/Ws</td><td>Power Consumption={$row['powerConsumption']}Ws</td></td></tr>\n";
        }
...
<div class='card-body'>
    ...
    foreach ($rooms_data['lights'] as $key => $row) {
        if ($row != null) {
            ...
            $color = $row['status'] == 1 ? "green" : "red";
            $returner['data'] .=  "<tr class='text-bold'><td>Light {$key} </td><td>Wattage={$row['Wattage']}Watts</td><td>Runtime={$row['runtime']}Seconds</td><td>Energy Cost={$row['energyCost']}p/Ws</td><td>Power Consumption={$row['powerConsumption']}Ws</td></td></tr>\n";
        }
    }
    $returner['data'] .= "</table>
</div>
