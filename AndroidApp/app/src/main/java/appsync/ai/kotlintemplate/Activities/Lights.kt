...
import appsync.ai.kotlintemplate.Adapters.LightsAdapter
...
class Lights() : AppCompatActivity() {
    ...
    lateinit var adapter: LightsAdapter
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        Admin.HandleToolBar(appContext, "Lights", go_back_img, title_head_txt)
...
//        Log.wtf("Thor-" + javaClass.name + "-" + Admin.lineNumber,"" + arrayList[1]::class.java.typeName)
        ...
        for (i in 1 until jsonArray.length()) {
            ...
            list.add(LightsReq("Light " + i, status.getString("status").toInt(), status.getDouble("runtime"), status.getLong("on_time"),status.getDouble("powerConsumption")))
        }
